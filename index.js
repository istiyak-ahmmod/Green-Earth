  ...
  fetch("https://openapi.programming-hero.com/api/categories")
    ...
    .then(data => {
      ...
      btnCategories.innerHTML += `
        <li id="all" class="text-[#1F2937] text-left font-bold cursor-pointer px-2 py-1 bg-[#15803D] ">All Trees</li>
      `;

      categories.forEach(cat => {
        btnCategories.innerHTML += `
          <li id="${cat.id}" class="hover:bg-[#15803D] hover:text-white text-[#1F2937] text-left font-bold  cursor-pointer px-2 py-1">${cat.category_name}</li>`;
      });
...
const categoryClickEvent = () => {
  btnCategories.addEventListener("click", (e) => {
    if (e.target.localName === "li") {
      ...
      document.querySelectorAll("#btn-categories li").forEach(li => li.classList.remove("bg-[#15803D]"));
...
loadingCatagory(false)
  plants.forEach(plant => {
    plantsByCategory.innerHTML += `
      <div class="bg-white p-4 rounded-lg shadow cursor-pointer" data-id="${plant.id}" data-title="${plant.name}" data-price="${plant.price}">
        ...
        <button class="w-full mt-4 bg-green-600 text-white px-4 py-1 rounded-full hover:bg-green-700 add-to-cart-btn cursor-pointer">Add to Cart</button>
      </div>`;
  });
};
  ...
  myCart.forEach((item, index) => {
    YourCartContainer.innerHTML += `
      <div class="flex justify-between bg-[#DCFCE7] rounded-xl items-center my-2 p-2 mb-2">
        ...
        <button onclick="removeFromCart(${index})" class="text-red-500 text-bold text-xl ml-2 cursor-pointer">×</button>
      </div>`;
    total += item.price;
  });
...
<div class="flex justify-center rounded-xl mb-5 mt-4"><img class="h-[250px] object-cover rounded-xl w-full" src="${data.plants.image}" alt=""></div>
